...
#+END_QUOTE
It then flows down some distance /d/ and we recalculate the fluid potential:
#+BEGIN_QUOTE
\Phi{}_{d} = g(h-d) + (p_{d} - p_{start})/\rho{}
#+END_QUOTE
/p_{d}/ is the pressure at the new location. This pressure has increased by the added weight above. This is the density of the fluid times the height: /\rho{}g(h-d)/. Looking at the units it's clear this gives a Pressure: /kg/m^3 \times{} m = kg/m^{2}/. The total pressure is just the sum of the previous pressure and the increase
#+BEGIN_QUOTE
p_{i} = p_{start} + \rho{}gd
#+END_QUOTE
We recalculate the fluid potential
#+BEGIN_QUOTE
\Phi{}_{d} = g(h-d) + (p_{start} + \rho{}gd - p_{start})/\rho{} = g(h-d) + gd = gh
#+END_QUOTE
...
gh = g(h-d) + gd
#+END_QUOTE
The two factors are called the /h-d/ *elevantion head* and /d/ *pressure head*.
...
** Darcy's Law
What induces a fluid to start flowing in a dynamic difference in energy between two points. Particles move from an area of high energy to an area of low energy. In other words from an area of high head to low head. The higher the difference the faster the flow. This is analogous to electric voltage
